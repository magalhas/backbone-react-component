...
(function (root, factory) {
  ...
  /** @lends Backbone.React.Component.prototype */
  {
     ...
     */
    __onError__: function (modelOrCollection, res, options) {
     ...
     */
    __onRequest__: function (modelOrCollection, xhr, options) {
      // Set props only if there's no silent option
      if (!options.silent)
        this.__setProps__({
          isRequesting: true,
     ...
     */
    __onSync__: function (modelOrCollection, res, options) {
      // Set props only if there's no silent option
      if (!options.silent)
    ...
    __startCollectionListeners__: function (collection, key) {
      ...
      if (collection instanceof Backbone.Collection)
        this
          .listenTo(collection, 'add remove change sort', _.bind(this.__setPropsBackbone__, this, collection, key, void 0))
          .listenTo(collection, 'error', this.__onError__)
          .listenTo(collection, 'request', this.__onRequest__)
          .listenTo(collection, 'sync', this.__onSync__);
      else if (collection)
    ...
    __startModelListeners__: function (model, key) {
      ...
      if (model instanceof Backbone.Model)
        this
          .listenTo(model, 'change', _.bind(this.__setPropsBackbone__, this, model, key, void 0))
          .listenTo(model, 'error', this.__onError__)
          .listenTo(model, 'request', this.__onRequest__)
          .listenTo(model, 'sync', this.__onSync__);
      else if (model)
        for (key in model)
